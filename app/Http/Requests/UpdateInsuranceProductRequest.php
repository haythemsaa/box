...
class UpdateInsuranceProductRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Warn if trying to deactivate a product with active subscriptions
            $insuranceProduct = $this->route('insurance_product');
            if ($insuranceProduct && $this->has('is_active') && !$this->is_active) {
                $activeSubscriptions = $insuranceProduct->contractInsurances()->active()->count();
                if ($activeSubscriptions > 0) {
                    $validator->errors()->add(
                        'is_active',
                        "Ce produit a {$activeSubscriptions} souscription(s) active(s). La désactivation empêchera de nouvelles souscriptions."
                    );
                }
            }
        });
